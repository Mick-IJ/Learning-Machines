...
class Environment:
    ...
    def play(self, q_table, n_its=100):
        ...
        for i in range(n_its):
            ...
            if (min(sensors) < 0.2) and not o:
                ...
                #break
                fitness += reward
                fit_hist.append(fitness)
            else:
                if n_collected_food + 1 == self.rob.collected_food():
                    reward = 50
                    n_collected_food += 1
    ...
    def get_state(self):
        ...
        t = 1 if min([self.time_since_obj_L, self.time_since_obj_R]) < 10 else 0
        l_recent = 1 if self.time_since_obj_L <= self.time_since_obj_R else 0

        return int(str(l_recent)+str(t)+str(o)+str(l)+str(r), 2), sensors, o
    ...
    def detect_object(self):
        ...
        if contoursL or contoursR:
            if contoursL and contoursR:
                ...
                self.time_since_obj_R = 0
                self.time_since_obj_L += 1
            else:  # contoursL
                self.time_since_obj_L = 0
                self.time_since_obj_R += 1
...
ind = Individual()
q_table = (np.array([  # Left, Right, Forward // # L_recent, T, O, L, RF
...
fit, fit_hist = ENV.play(ind.q_table, n_its=5000)
